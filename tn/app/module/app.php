...
<html>
    <head>
      ...
      <?php
        $array_dataTables = array($def['get_link_block'], $def['get_link_department'], $def['link_title'], $def['get_link_profile'], $def['get_link_exam'], $def['get_link_each_exam'], $def['get_link_question'], $def['link_knowledge'], $def['link_profile_exam'], $def['link_profile_exam'], $def['link_construct_exam']);
                ...
                $(document).on('click', '#submit_import', function(){
                    ...
                    $('#form_jquery').ajaxForm({
                        ...
                        complete: function(xhr) {
                            ...
                            var n = text.split(";");
                            if(n[0] == '1'){
                                toastr.success('<?php echo $lang['import_question_success'] ?>');
                                setTimeout(function(){
                                    window.location.replace("<?php echo $def['link_question']; ?>");
                                }, 1000);
                            } else {           
