...
class mysql 
{
	...
	function login_user($user, $password, $table = 'profiles', $column_user = 'profile_code', $column_password = 'password')
    {
       ...
       $su = $this->query("select id, block_id, department_id, title_id, profile_code, password, fullname, birthday, role, active from $table where $column_user = '$user' and active = 1 and deleted_at is null");
       $nu = $this->num_rows($su);
       if($nu){
    ...
    function check_info($user, $table = 'profiles', $column_user = 'profile_code')
    {
       $su = $this->query("select id, block_id, department_id, title_id, profile_code, password, fullname, birthday, role, active from $table where $column_user = '$user' and active = 1 and active_exam = 1 and deleted_at is null");
       $nu = $this->num_rows($su);
       if($nu){
            $ru = $this->fetch_array($su);
            $_SESSION['is_logined_exam'] = $ru;
            echo '1';
       } else echo '2';
	}
    ...
    {
        $data = array();
        $s = $s = $this->query("select * from qandas where knowledge = '$knowledge' and deleted_at is null order by id desc");
        while($r = $this->fetch_array($s)) {
            array_push($data, $r);
        }
        return $data;
    ...
    // get number profiles by code
    function getNumberProfileByCode($profile_code)
    {
        $s = $this->query("select id from profiles where profile_code = '$profile_code' and deleted_at is null");
        $n = $this->num_rows($s);
        return $n;
    }
    ...
    {
        $data = array();
        $s = $this->query("select id, name, start_date, end_date, exam_time, exam_code, result, point, created_by from exams where deleted_at is null order by id desc");
        while($r = $this->fetch_array($s)) {
            array_push($data, $r);
        }
        return $data;
    ...
    function getExamsBlock($exam_id)
    {
        $data = array();
        $s = $this->query("select * from exam_block where exam_id = $exam_id and deleted_at is null order by id desc");
        while($r = $this->fetch_array($s)) {
            array_push($data, $r);
        }
        return $data;
    ...
    // get data exams block
    function getExamsBlockBDT($exam_id, $block_id, $department_id, $title_id)
    {
        $data = array();
        $s = $this->query("select * from exam_block where exam_id = $exam_id and block_id = $block_id and department_id = $department_id and title_id = $title_id and deleted_at is null order by id desc");
        while($r = $this->fetch_array($s)) {
            array_push($data, $r);
        }
        return $data;
    ...
    function getResultsByExamProfile($exam_id, $profile_id)
    {
        $wh = "exam_id = $exam_id and profile_id = $profile_id and deleted_at is null";
        $s = $this->query("select * from results where $wh order by exam_id desc, point desc, id desc");
        $r = $this->fetch_array($s);
        return $r;
    }
    // checkExistResultEP
    function checkExistResultEP($exam_id, $profile_id)
    {
        $wh = "exam_id = $exam_id and profile_id = $profile_id and deleted_at is null";
        $s = $this->query("select * from results where $wh");
        $n = $this->num_rows($s);
        return $n;
    }
    // get results follow exam
    function getResultsByExam($exam_id)
    {
        ...
        if ($exam_id == '') 
            ...
            $wh = "exam_id = $exam_id and deleted_at is null";
        $s = $this->query("select * from results where $wh order by exam_id desc, point desc, id desc");
        while($r = $this->fetch_array($s)) {
            array_push($data, $r);
        }
    ...
    function getResultsByExamDepartment($exam_id, $block_id, $department_id)
    {
        $data = array();
        if ($exam_id == '') 
            $wh = "block_id = $block_id and department_id = $department_id and deleted_at is null";
        else
            $wh = "exam_id = $exam_id and block_id = $block_id and department_id = $department_id and deleted_at is null";
        $s = $this->query("select * from results where $wh order by exam_id desc, point desc, id desc");
        while($r = $this->fetch_array($s)) {
            array_push($data, $r);
        }
    ...
    function getResultsPassedByExam($exam_id)
    {
        $data = array();
        if ($exam_id == '') {
            $exam = $this->getExams();
            if (count($exam)) {
                foreach ($exam as $ex) {
                    $idx = $ex['id'];
                    $point = $ex['point'];
                    $wh = "exam_id = $idx and point >= $point  and deleted_at is null";
                    $s = $this->query("select * from results where $wh order by exam_id desc, point desc, id desc");
                    while($r = $this->fetch_array($s)) {
                        array_push($data, $r);
                    }
            ...
            $ex = $this->getExamById($exam_id);
            $point = $ex['point'];
            $wh = "exam_id = $exam_id and point >= $point  and deleted_at is null";
            $s = $this->query("select * from results where $wh order by exam_id desc, point desc, id desc");
            while($r = $this->fetch_array($s)) {
                array_push($data, $r);
            }
    ...
    function getResultsPassedByExamDepartment($exam_id, $block_id, $department_id)
    {
        $data = array();
        if ($exam_id == '') {
            $exam = $this->getExams();
            if (count($exam)) {
                foreach ($exam as $ex) {
                    $idx = $ex['id'];
                    $point = $ex['point'];
                    $wh = "exam_id = $idx and block_id = $block_id and department_id = $department_id and point >= $point and deleted_at is null";
                    $s = $this->query("select * from results where $wh order by exam_id desc, point desc, id desc");
                    while($r = $this->fetch_array($s)) {
                        array_push($data, $r);
                    }
            ...
            $ex = $this->getExamById($exam_id);
            $point = $ex['point'];
            $wh = "exam_id = $exam_id and block_id = $block_id and department_id = $department_id and point >= $point and deleted_at is null";
            $s = $this->query("select * from results where $wh order by exam_id desc, point desc, id desc");
            while($r = $this->fetch_array($s)) {
                array_push($data, $r);
            }
    ...
    function getResultsFailedByExam($exam_id)
    {
        $data = array();
        if ($exam_id == '') {
            $exam = $this->getExams();
    ...
    function getResultsFailedByExamDepartment($exam_id, $block_id, $department_id)
    {
        $data = array();
        if ($exam_id == '') {
            $exam = $this->getExams();
    ...
    function getExamProfiles($profile_id)
    {
        ...
        if ($ns) {
            ...
            while ($r = $this->fetch_array($s)){
                $exam_id = $r['exam_id'];
                $se = $this->query("select result from exams where id = $exam_id");
                $re = $this->fetch_array($re);
                ...
                if ($nr) {
                    ...
                    $data1 = array("id"=>$r['id'], "exam_id" => $r['exam_id'], "block_id" => $r['block_id'], "department_id" => $department_id, "title_id" => $title_id, "start_date" => $r['start_date'], "end_date" => $r['end_date'], 'time_exam' => $r['time_exam'], "number_questions" => $rr['number_questions'], "number_answer_right" => $rr['number_answer_right'], "point" => $rr['point'], "result" => $re['result']);    
                } else {
                    $data1 = array("id"=>$r['id'], "exam_id" => $r['exam_id'], "block_id" => $r['block_id'], "department_id" => $department_id, "title_id" => $title_id, "start_date" => $r['start_date'], "end_date" => $r['end_date'], 'time_exam' => $r['time_exam'], "number_questions" => 0, "number_answer_right" => 0, "point" => 0, "result" => $re['result']);
                }
    ...
    // get QuizNotDone
    function getQuizNotDone($profile_id)
    {
        $data = array();
        $ss = $this->query("select block_id, department_id, title_id from profiles where id = $profile_id and deleted_at is null");
        $ns = $this->num_rows($ss);
        if ($ns) {
            ...
            $ttss = $title_id.'-'.$tt['name'];
            //$s = $this->query("select exam_id, block_id, start_date, end_date, number_questions, number_answer_right, point from exam_block as e INNER JOIN results as r ON e.exam_id = r.exam_id and e.block_id = r.block_id where e.department_id like '%$dpss%' and title_id like '%$ttss%' and e.deleted_at is null");
            $s = $this->query("select id, exam_id, block_id, department_id, start_date, time_exam from exam_block where block_id = $block_id and department_id like '%$dpss%' and title_id like '%$ttss%' and deleted_at is null order by exam_id desc");
            $exams = '';
            while ($r = $this->fetch_array($s)){
                $exam_id = $r['exam_id'];
                $exams .= $exam_id.',';
            ...
            if ($exams != '') {
                ...
                $today = date("Y-m-d");
                $se = $this->query("select * from exams where id IN ($exams) and deleted_at is null order by id desc");
                while ($re = $this->fetch_array($se)) {
                    $start_date = date("Y-m-d", $re['start_date']);
                    $end_date = date("Y-m-d", $re['end_date']);
    ...
    // get knowledge by nam
    function getKnowledgeByName($name)
    {
        $s = $this->query("select id, name from knowledges where name = '$name' and deleted_at is null order by id desc");
        $r = $this->fetch_array($s);
        return $r;
    }
    // get knowledge by id
    ...
    // get ConstructExam By Exam and Title
    function getConstructExamByExamIdAndTitleId($exam_id, $title_id)
    {
        $data = array();
        $s = $this->query("select id, title_id, construct from construct_exam where active = 1 and exam_id = $exam_id and title_id like '%$title_id%' order by id");
        $n = $this->num_rows($s);
        if ($n) {
            while ($r = $this->fetch_array($s)) {
                array_push($data, $r);
    ...
    // check exist exam_profile
    function checkExistExamProfile($exam_id, $profile_id)
    {
        $s = $this->query("select id from exam_profile where exam_id = $exam_id and profile_id = $profile_id order by id");
        $n = $this->num_rows($s);
        return $n;
    }
    // check exist exam_questions
    function checkExistExamQuestions($exam_id, $profile_id)
    {
        $s = $this->query("select id from exam_questions where exam_id = $exam_id and profile_id = $profile_id order by id");
        $n = $this->num_rows($s);
        return $n;
    }
    // get exam_profile
    function getExamProfileExamIdProfileId($exam_id, $profile_id)
    {
        $s = $this->query("select * from exam_profile where exam_id = $exam_id and profile_id = $profile_id order by id");
        $r = $this->fetch_array($s);
        return $r;
    }
    // get question limit follow knowledge
    function getQuestionKnowledgeLimit($knowledge, $limit)
    {
        $data = array();
        $s = $this->query("select id, type_question, knowledge, question, image, answer, right_answer from qandas where knowledge = '$knowledge' and deleted_at is null order by rand() limit 0, $limit");
        $n = $this->num_rows($s);
        if ($n) {
            while ($r = $this->fetch_array($s)) {
                array_push($data, $r);
    ...
    // get question follow exam_id, profile_id
    function getQuestionsProfileExam($exam_id, $profile_id)
    {
        $data = array();
        $s = $this->query("select * from exam_questions where exam_id = $exam_id and profile_id = $profile_id order by id");
        $n = $this->num_rows($s);
        if ($n) {
            while ($r = $this->fetch_array($s)) {
                array_push($data, $r);
    ...
    // get exam_questions by id
    function getExamQuestionsProfileExam($exam_id, $profile_id, $question_id)
    {
        $s = $this->query("select * from exam_questions where exam_id = $exam_id and profile_id = $profile_id and question_id = $question_id order by id");
        $r = $this->fetch_array($s);
        return $r;
    }
    // get right_answer_number
    function getNumberRightAnswer($ids = array(), $exam_id, $profile_id)
    {
        $number = 0;
        foreach ($ids as $i => $id) {
            $eq = $this->getExamQuestionsProfileExam($exam_id, $profile_id, $id);
            $profile_answer = $eq['profile_answer'];
            $s = $this->query("select id from qandas where id = $id and right_answer = '$profile_answer'");
            $n = $this->num_rows($s);
            if ($n)
                $number++;
            else
        ...
        $r = $this->getResultsByExamProfile($exam_id, $profile_id);
        $idr = $r['id'];
        $number_answer_right = $number;
        $number_questions = $r['number_questions'];
        $point = round($number_answer_right/$number_questions, 3) * 10;
        $data2['number_answer_right'] = $number_answer_right;
        $data2['point'] = $point;
        $table2 = "results";
        $where2 = " where id = $idr";
        $sr = $this->update($data2, $table2, $where2);
    }
}
