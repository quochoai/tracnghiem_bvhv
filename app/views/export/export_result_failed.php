            ...
            <h3 class="card-title">
                <?php 
                    if ($role == $def['role_organizer']) 
                        echo $lang['export_result_exam_failed'];
                    else {
                        $block = $h->getBlockById($profile['block_id']);
                        $dp = $h->getDepartmentById($profile['department_id']);
                        echo $lang['export_result_exam_failed'].' - '.$lang['block_name'].': '.$block['name'].' - '.$lang['department_exam'].': '.$dp['name']; 
                    }
                ?>
            </h3>
          </div>
          <!-- /.card-header -->
          <form action="<?php echo $def['link_process_export_result_failed'] ?>" method="post" id="form_jquery" role="form" enctype="multipart/form-data">
